...
function getChartOption({option, type}) {
  ...
  if (type === 'latency') {
    return {
      ...
      xAxis: {
        ...
        nameLocation: 'middle',
        nameGap: 30,
        splitLine: {lineStyle: {color: 'var(--hk-wire-faint)'}},
      },
      yAxis: {
        type: 'category',
        data: ['pccx (FPGA)', 'llm-bottleneck-lab (CPU)', 'GPU Baseline'],
        axisLine: {lineStyle: {color: 'var(--hk-wire-ink)'}},
      },
      series: [
        {
          name: 'Prefill',
      ...
      <div className="hk-frame__body" style={{padding: 'var(--hk-s-5)'}}>
        <BrowserOnly fallback={<div>Loading chart...</div>}>
          {() => {
            ...
            return <ReactECharts option={getChartOption({option, type})} style={{height, width: '100%'}} opts={{renderer: 'svg'}} />;
          }}
        </BrowserOnly>
      </div>
    </div>
